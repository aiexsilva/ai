...
# --- 1. SETUP GLOBAL (Corre apenas uma vez no arranque) ---
print("A carregar modelo NLP...")
# Usamos um modelo leve e rápido
model = SentenceTransformer('all-MiniLM-L6-v2')

# As tuas categorias e palavras-chave de "backup" ou definição semântica
...
# Pré-calculamos os vetores das categorias para ser super rápido na execução
# Assim não recalculamos isto a cada pedido do utilizador
CATEGORY_EMBEDDINGS = model.encode(CATEGORIES, convert_to_tensor=True)
print("Modelo e categorias carregados com sucesso.")

def get_best_category_nlp(user_text):
    print(f"Processing Input: '{user_text}'")
    
    # Converte o texto do user em vetor
    user_embedding = model.encode(user_text, convert_to_tensor=True)
    
    # Calcula a similaridade (Matemática)
    cosine_scores = util.cos_sim(user_embedding, CATEGORY_EMBEDDINGS)[0]

    # Vê qual é a pontuação mais alta
    ...
    print(f"Match: '{best_category}' | Score: {best_score:.4f}")
    
    return best_category

@functions_framework.http
def match_keywords(request):
    # Setup de CORS (Para o Flutter não reclamar)
    ...
    # TRUQUE: Juntamos a lista numa frase só.
    # Ex: ["fome", "jantar"] vira "fome jantar"
    # A AI percebe muito melhor o contexto assim do que palavras soltas.
    query_text = " ".join(keywords)

    try:
        best_category = get_best_category_nlp(query_text)
